...
return locataires;
	...
	public ArrayList<Locataire>getByKeywordsAndByIdAgent(String keyword,int id){
		ArrayList<Locataire>locataires=new ArrayList<Locataire>();
		try {
			PreparedStatement ps = Database.connexion.prepareStatement("SELECT * FROM locataire WHERE nom LIKE? OR prenom LIKE ? or ville LIKE ? WHERE id IN(SELECT id_locataire FROM contratl WHERE id_bien IN(SELECT id FROM bien WHERE id_agent=?))");
